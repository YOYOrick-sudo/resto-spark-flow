...
-- ============================================================
-- LOOP 4: Verpakking-conversie + learning loop
-- ============================================================

-- 1. Mini-schema-uitbreiding goods_receipt_lines
ALTER TABLE public.goods_receipt_lines
  ADD COLUMN IF NOT EXISTS leverancier_artikel_id uuid
    REFERENCES public.leveranciers_artikelen(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS werkelijk_gewicht_g numeric;

CREATE INDEX IF NOT EXISTS idx_goods_receipt_lines_la_id
  ON public.goods_receipt_lines(leverancier_artikel_id)
  WHERE leverancier_artikel_id IS NOT NULL;

-- 2. Nieuwe confirm_goods_receipt RPC
...
BEGIN
  ...
  LOOP
    ...
    -- Pre-computed by edge function (or NULL als geen voorraad-mutatie)
    ...
    v_should_mutate_stock := (v_status = 'akkoord' OR v_accepted_with_issue)
                              AND v_line_db.ingredient_id IS NOT NULL;

    -- Hard fail (defense-in-depth): geen stille foutieve voorraad
    ...
    -- Update lijnregel (incl. la_id en werkelijk_gewicht_g)
    ...
    IF v_should_mutate_stock THEN
      SELECT id, eenheid, base_unit
      INTO v_ingredient
      FROM ingredienten
      WHERE id = v_line_db.ingredient_id;

      -- a) Nieuwe append-only stock_movements
      INSERT INTO stock_movements (
        location_id, ingredient_id, reason, delta_base, base_unit,
        source_type, source_ref, lotnummer, tht_datum, user_id, note
      ) VALUES (
        v_receipt.location_id,
        ...
        'goods_receipt_line',
        v_line_db.id,
        COALESCE(v_line_input->>'lotnummer', v_line_db.lotnummer),
      ...
      -- b) Nieuwe stock_lots (voor lot-tracking + variabel gewicht)
      INSERT INTO stock_lots (
        location_id, ingredient_id, qty_pieces, qty_base, base_unit,
        weight_per_piece_g_at_receipt, lotnummer, tht_datum,
        source_type, source_ref, received_at
      ) VALUES (
        v_receipt.location_id,
        v_line_db.ingredient_id,
        CASE WHEN v_base_unit = 'st' THEN v_delta_base ELSE NULL END,
        v_delta_base,
        v_base_unit,
        v_werkelijk_gewicht_g,
        COALESCE(v_line_input->>'lotnummer', v_line_db.lotnummer),
        COALESCE((v_line_input->>'tht_datum')::date, v_line_db.tht_datum),
        'goods_receipt_line',
        v_line_db.id,
      ...
      -- c) LEGACY parallel — voorraad_bewegingen + ingredienten.voorraad
      -- Gebruik delta_base (in ingredient.eenheid wanneer dat == base_unit, anders raw qty als veilige fallback)
      v_legacy_qty := CASE
        WHEN v_ingredient.base_unit IS NOT NULL AND v_ingredient.base_unit = v_base_unit
          THEN v_delta_base
        ELSE v_qty   -- fallback voor zeldzame mismatch-gevallen
      END;
      ...
      UPDATE ingredienten
      SET voorraad = COALESCE(voorraad, 0) + v_legacy_qty,
          updated_at = now()
      WHERE id = v_line_db.ingredient_id;

      v_voorraad_movements := v_voorraad_movements + 1;

      -- d) LEARNING LOOP: confirmation_count++ en factor_source promotion
      IF v_la_id IS NOT NULL THEN
        ...
        SET
          ...
          factor_source = CASE
            -- 'user' wint altijd (chef-input is bron van waarheid)
            WHEN factor_source = 'user' THEN 'user'
            -- expliciet door edge function gezet (manual / ai_confirmed)
    ...
    IF v_status <> 'akkoord' THEN
      ...
      INSERT INTO credit_note_requests (
        ...
        CASE
          WHEN v_accepted_with_issue
            THEN format('GEACCEPTEERD: %s', COALESCE(v_line_input->>'afwijking_notitie',''))
          ELSE v_line_input->>'afwijking_notitie'
        END,
        v_dedup_key
      )
  ...
  ----------------------------------------------------------
  -- TEMPERATUUR (ongewijzigd t.o.v. v1)
  ----------------------------------------------------------
  ...
  ELSIF _temp_skip ? 'gekoeld' THEN
    ...
    VALUES (v_checklist_run_id, 'temp_gekoeld', 'notitie',
  ...
  ELSIF _temp_skip ? 'vries' THEN
    INSERT INTO checklist_responses (run_id, item_id, type, notitie, ingevuld_door, ingevuld_op)
    VALUES (v_checklist_run_id, 'temp_vries', 'notitie',

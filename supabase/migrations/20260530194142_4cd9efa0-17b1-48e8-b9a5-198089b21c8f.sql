  ...
  LOOP
    ...
    IF v_should_mutate_stock THEN
      SELECT EXISTS (
        ...
        WHERE referentie_type = 'goods_receipt_line'
          AND referentie_id = v_line_db.id
      ...
      IF NOT v_already_booked THEN
        INSERT INTO voorraad_bewegingen (
          ...
          v_line_db.id,
          v_profile_user_id,
          format('Pakbon-ontvangst receipt %s regel %s (%s %s)', _receipt_id, v_line_db.id, v_delta_base, v_base_unit)
        );
  ...
  IF v_template_id IS NOT NULL THEN
    ...
    IF v_checklist_run_id IS NULL THEN
      ...
      ON CONFLICT (template_id, location_id, datum) DO UPDATE
        SET status = 'afgerond',
            afgerond_op = now(),
            afgerond_door = EXCLUDED.afgerond_door,
            updated_at = now()
    ...
    ELSE
      ...
      SET status = 'afgerond',
          afgerond_op = now(),
          afgerond_door = v_profile_user_id,
    ...
    IF _temp_gekoeld IS NOT NULL THEN
      INSERT INTO checklist_responses (
        ...
        'temperatuur',
        _temp_gekoeld,
        (_temp_gekoeld <= 7) AND NOT v_strict_temp_alarm,
        v_profile_user_id,
        now()
      )
    ...
    ELSIF _temp_skip ? 'gekoeld' THEN
      INSERT INTO checklist_responses (
        ...
        'temp_gekoeld',
        'notitie',
        format('OVERGESLAGEN: %s', _temp_skip->>'gekoeld'),
        v_profile_user_id,
        now()
      )
      ON CONFLICT DO NOTHING;
    ...
    ELSIF _temp_skip ? 'vries' THEN
      INSERT INTO checklist_responses (
        ...
        'temp_vries',
        'notitie',
        format('OVERGESLAGEN: %s', _temp_skip->>'vries'),
        v_profile_user_id,
        now()
      )
      ON CONFLICT DO NOTHING;

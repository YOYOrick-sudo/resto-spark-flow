...
BEGIN
  ...
  LOOP
    ...
    -- accepted_with_issue alleen geldig bij beschadigd/verkeerd
    ...
    -- Voorraad-mutatie: bij akkoord OF accepted_with_issue
    IF (v_status = 'akkoord' OR v_accepted_with_issue) AND v_line_db.ingredient_id IS NOT NULL THEN
      INSERT INTO voorraad_bewegingen (
        ...
        _user_id,
        CASE
          WHEN v_accepted_with_issue THEN format('Pakbon %s (geaccepteerd met afwijking: %s)',
            COALESCE(v_receipt.pakbon_nummer, _receipt_id::text), v_status)
          ELSE format('Pakbon %s', COALESCE(v_receipt.pakbon_nummer, _receipt_id::text))
        END
      );

      UPDATE ingredienten
      SET voorraad = voorraad + v_qty,
          updated_at = now()
    ...
    -- Credit-note: bij elke afwijking (incl. accepted_with_issue)
    IF v_status <> 'akkoord' THEN

-- Sprint C2b — Edge case: items zonder eigen freq in per_item modus uitsluiten
-- Reden: in per_item is item-level control de bedoeling. Items zonder freq = chef vergat config → niet stilletjes erven.
...
BEGIN
  ...
  LOOP
    ...
    ELSIF tpl.modus = 'per_item' THEN
      -- Per item: alleen items mét eigen frequentie tellen mee
      due_items := '[]'::jsonb;
      FOR itm IN SELECT * FROM jsonb_array_elements(tpl.items)
      LOOP
        -- EDGE CASE: skip items zonder eigen frequentie in per_item modus

...
-- Fix: add task_type to existing task_templates that are missing it
UPDATE onboarding_phases
SET task_templates = (
  SELECT jsonb_agg(
    CASE
      WHEN elem->>'task_type' IS NOT NULL THEN elem
      WHEN lower(elem->>'title') ~ '(bevestiging|email|sturen|herinnering|reminder|uitnodiging|welkom)'

...
export function validateFactuur(data: FactuurV2Output): ValidationResult {
    ...
    if (
      ...
      if (Math.abs(verwacht - regel.prijs_totaal) > CENT_TOLERANCE) {
        warnings.push(
          `Regel ${
            idx + 1
          } (${regel.product_naam}): ${regel.hoeveelheid_besteld} × ${regel.prijs_per_besteld_item} = ${
            verwacht.toFixed(2)
          }, factuur zegt ${regel.prijs_totaal}`,
        );
      }
    }
  }
  ...
  if (data.subtotaal_excl_btw != null) {
    ...
    if (Math.abs(verwachtSubtotaal - data.subtotaal_excl_btw) > SUM_TOLERANCE) {
      warnings.push(
        `Som regels (${
          verwachtSubtotaal.toFixed(2)
        }) wijkt af van subtotaal (${data.subtotaal_excl_btw})`,
      );
    }
  }

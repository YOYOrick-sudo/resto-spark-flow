...
async function handleBook(body: Record<string, unknown>, clientIp: string, req: Request) {
  ...
  if (ticketData?.policy_set_id) {
    const { data: policySet } = await admin
      .from('policy_sets')
      .select('payment_type, payment_amount_cents')
      .eq('id', ticketData.policy_set_id)
      .single();

    if (policySet && policySet.payment_type !== 'none' && policySet.payment_amount_cents && policySet.payment_amount_cents > 0) {
      requiresPayment = true;
      paymentAmountCents = policySet.payment_amount_cents;
    }
  }

  // 5. Create reservation via RPC
  ...
  // 8. Handle marketing opt-in (non-blocking)
  ...
  // 9. Send confirmation email — only if NO payment required
  // When payment is required, the mollie-webhook sends the confirmation after payment
  if (!requiresPayment) {
    try {
  ...
  // Add payment info when deposit is required
  if (requiresPayment) {
    response.requires_payment = true;
    response.payment_info = {
      amount_cents: paymentAmountCents * party_size,
      amount_per_person_cents: paymentAmountCents,
      currency: 'EUR',
    };
  }
...
// ============================================
// Booking Confirmation Email — imported from _shared/bookingEmail.ts
// ============================================
// (sendBookingConfirmationEmail is now imported at the top)

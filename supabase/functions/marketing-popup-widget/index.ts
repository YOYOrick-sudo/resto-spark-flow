    ...
    fetch(CONFIG_URL).then(function(r){return r.json()}).then(function(cfg){
      ...
      styles.textContent=\`
        *{box-sizing:border-box;margin:0;padding:0;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;}
        ...
        .nesto-bar{position:fixed;left:0;right:0;z-index:999997;padding:14px 80px;display:flex;align-items:center;justify-content:space-between;gap:12px;transform:translateY(100%);transition:transform .3s ease;}
        ...
        .nesto-bar .nesto-btn{padding:6px 14px;font-size:13px;background:rgba(255,255,255,0.15);border:1.5px solid #fff;color:#fff;}
        .nesto-bar .nesto-btn:hover{background:rgba(255,255,255,0.25);}
        .nesto-bar-close{position:absolute;right:16px;top:50%;transform:translateY(-50%);background:none;border:none;font-size:18px;cursor:pointer;color:#fff;padding:4px;opacity:0.8;}

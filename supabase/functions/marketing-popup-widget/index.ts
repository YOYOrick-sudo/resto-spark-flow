    ...
    fetch(CONFIG_URL).then(function(r){return r.json()}).then(function(cfg){
      ...
      styles.textContent=\`
        *{box-sizing:border-box;margin:0;padding:0;font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif;}
        ...
        .nesto-bar{position:fixed;left:0;right:0;z-index:999997;padding:14px 120px;display:flex;align-items:center;justify-content:space-between;gap:12px;transform:translateY(100%);transition:transform .3s ease;}
        ...
        .nesto-bar-text{flex:1;font-size:15px;font-weight:600;color:#fff;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;}
        ...
        .nesto-bar .nesto-btn{padding:5px 12px;font-size:13px;background:rgba(255,255,255,0.15);border:1.5px solid #fff;color:#fff;}
        .nesto-bar .nesto-btn:hover{background:rgba(255,255,255,0.25);}
        .nesto-bar-close{position:absolute;right:16px;top:50%;transform:translateY(-50%);background:none;border:none;font-size:22px;cursor:pointer;color:#fff;padding:4px;opacity:0.8;}
        .nesto-bar-close:hover{opacity:1;}

        @media(max-width:600px){
          .nesto-form{flex-direction:column;}
          .nesto-bar{flex-direction:column;text-align:center;padding:14px 24px;}
          .nesto-bar .nesto-input{width:100%;flex:1;}
          .nesto-bar-text{width:100%;}
          .nesto-bar-close{top:8px;right:8px;transform:none;}
        }
      \`;

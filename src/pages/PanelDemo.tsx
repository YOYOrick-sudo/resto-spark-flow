...
function PanelShell({ 
  ...
  renderContent: (titleRef: React.RefObject<HTMLHeadingElement>) => React.ReactNode;
}) {
    ...
    <PanelShell
      ...
      renderContent={(titleRef) => (
        <div className="p-5 pr-14 space-y-5">
          <h2 ref={titleRef} className="text-lg font-semibold text-foreground">Nieuwe reservering</h2>
          ...
          <div>
            <label className="text-[13px] font-medium text-muted-foreground mb-1.5 block">Aantal gasten</label>
            <div className="flex items-center gap-3">
              <button 
                onClick={() => setGuests(Math.max(1, guests - 1))}
                className="h-10 w-10 rounded-control border border-input bg-background flex items-center justify-center hover:bg-secondary transition-colors"
              >
                <Minus className="h-4 w-4" />
              </button>
              <div className="flex items-center gap-2">
                <Users className="h-4 w-4 text-muted-foreground" />
                <span className="text-lg font-semibold w-6 text-center tabular-nums">{guests}</span>
              </div>
              <button 
          ...
          <div>
            <label className="text-[13px] font-medium text-muted-foreground mb-1.5 block">Shift</label>
          ...
          <div className="border-t border-border/50 pt-5 mt-5">
            ...
            <div className="space-y-3">
              <div>
                <label className="text-[13px] font-medium text-muted-foreground mb-1 block">Naam</label>
                <input 
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="text-[13px] font-medium text-muted-foreground mb-1 block">Telefoon</label>
                  <input 
                    type="tel"
                    placeholder="+31 6..."
                    className="w-full px-3 py-2.5 rounded-control border border-input bg-background text-sm placeholder:text-muted-foreground/60 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-1"
                ...
                <div>
                  <label className="text-[13px] font-medium text-muted-foreground mb-1 block">E-mail</label>
                  <input 
                    type="email"
                    placeholder="gast@..."
                    className="w-full px-3 py-2.5 rounded-control border border-input bg-background text-sm placeholder:text-muted-foreground/60 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-1"
          ...
          <div>
            <label className="text-[13px] font-medium text-muted-foreground mb-1 block">Notities</label>
            <textarea
              rows={3}
    ...
    <div className="min-h-screen bg-background p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <p className="text-sm text-muted-foreground mb-8">
          Beide modes 460px breed. Titel scrollt mee; reveal header verschijnt wanneer de titel uit beeld verdwijnt.
        </p>
        ...
        <div className="flex gap-8 flex-wrap">
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm text-muted-foreground">460px · reveal header</span>
            </div>
            <DetailPanelDemo />
          </div>
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              ...
              <span className="text-sm text-muted-foreground">460px · reveal header + footer</span>
            </div>
            <FormPanelDemo />
          </div>
        </div>

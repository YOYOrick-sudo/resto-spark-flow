...
export function useWidgetButtonLogoUpload() {
  ...
  const uploadLogo = async (file: File) => {
    ...
    try {
      ...
      const path = `${locationId}/button-logo.${ext}`;

      // Remove any existing button logo files first
      const { data: existing } = await supabase.storage
        .from(BUCKET)
        .list(locationId, { limit: 50 });

      if (existing?.length) {
        const logoFiles = existing.filter((f) => f.name.startsWith('button-logo.'));
        if (logoFiles.length) {
      ...
      await updateSettings.mutateAsync({ widget_button_logo_url: logoUrl } as any);
      nestoToast.success('Button logo geüpload');
    } catch {
  ...
  const deleteLogo = async () => {
    ...
    try {
      ...
      await updateSettings.mutateAsync({ widget_button_logo_url: null } as any);
      nestoToast.success('Button logo verwijderd');
    } catch {

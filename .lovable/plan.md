...
# Voorraad & Inkoop Database — Migratie

## Overzicht

Eén SQL migratie die 5 nieuwe tabellen, 3 functies, 3 triggers, RLS policies en indexes aanmaakt.
...
-- Bestelnummer generatie: YYYY-XXX per location per jaar
...
-- Ontvangst trigger: voorraad_bewegingen + voorraad update
...
-- Waste trigger: voorraad_bewegingen + voorraad update
...
- **5 tabellen**: leveranciers, leveranciers_artikelen, bestellingen, bestelregels, waste_registraties
- **3 functies**: bestelnummer generatie (YYYY-XXX), ontvangst verwerking (voorraad IN), waste verwerking (voorraad WASTE)
- **3 triggers**: status validatie, ontvangst automatisering, waste automatisering
- **20 RLS policies**: location-based access via bestaande `user_has_location_access` en `user_has_role_in_location`
- **9 indexes**: op foreign keys en veelgebruikte query-patronen
- **3 updated_at triggers**: hergebruikt bestaande `update_updated_at_column()`

Geen frontend wijzigingen in deze stap — puur database schema.

## Bestand

| Bestand | Actie |
|---|---|
| Database migratie | Nieuw — volledige SQL hierboven |
